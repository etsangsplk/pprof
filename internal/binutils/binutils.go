...
var objdumpLLVMVerRE = regexp.MustCompile(`((?i)LLVM) ((?i)version) (\d*)\.(\d*)\.(\d*)`)
...
func initTools(b *binrep, config string) {
	...
	// Look for llvm-objdump and objdump.
	b.objdump, b.isLLVMObjdump, b.objdumpFound = findObjdump(append(paths["objdump"], defaultPath...))
}

// findObjdump finds and returns preferred objdump binary.
...
func isLLVMObjdump(output string) bool {
	...
	if len(fields) != 6 {
		// Check if built from trunk.
		output = strings.ToLower(output)
		return strings.Contains(output, "trunk") && strings.Contains(output, "llvm version")
	}
	verMajor, err := strconv.Atoi(fields[3])
	if err != nil {
		return false
	}
	verPatch, err := strconv.Atoi(fields[5])
	if err != nil {
		return false
	}
